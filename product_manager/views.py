...
def index(request):
    if request.method == "GET":
        ...
        if "-" in q:
            ...
            filter_query = Q(name__icontains=q) | Q(price__icontains=q) | Q(brand_id__name__icontains=q) | Q(category_id__name__icontains=q)
            products = Product.objects.filter(filter_query)
            categories = Category.objects.all()
            brands = Brand.objects.all()
            context = {
...
def cart(request):
    ...
    if product_id:
        ...
        #to prevent from entering -ve quantity
        if quantity:
            if int(quantity)<=0:
